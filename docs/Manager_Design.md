...
- Computing rent or taxes (engine/rules)
- Persisting game state (infrastructure)
- Handling HTTP requests (api layer)
...
new_turn(): Move to next turn's player
...
### State Variable: game_phase
- **What:**
    - Enum representing macro-phase of game (Setup, Playing, Ended)
- **Why:**
    - Used for additional validation to confirm permissibility of actions like buy, move, sell. 
- **How:**
    - Updated in create_game to SETUP, in start_game to PLAYING, in end_game to ENDED
    - Used for validation in methods start_game, move_player, land_on_space, handle_action, end_game
    - Used to confirm if game is over
### State Variable: turn_history (not included in mvp)
- **What:**
    - List of dictionaries each representing a turn taken in the game
    - A dictionary includes player, action type, affected space(s), timestamp, outcome
- **Why:**
    - Record useful for debugging, undo/redo features, logging for analytics
- **How:**
    - Updated in handle_action, move_player, land_on_space
...
### Method: start_game()
- **What:**
  - Method to begin gameplay. Changes phase from SETUP to PLAYING and initializes first turn.
- **Why:**
  - Separate setup from gameplay so validation and authorization checks are done before game-state changes occur.
- **Lives in:** `application/use_cases/start_game.py`
- **Calls into:** `engine/services/turn_resolution.py`
- **How:**
  - Validation: `game_phase = SETUP`, minimum required players, caller is authorized to start game
  - Randomize players order (turn order)
  - Set phase to playing
  - Set `index_of_current_player = 0`
  - Set `game_phase = PLAYING`
  - Call new_turn to initialize first-turn state
- **Failure behavior:**
  - Return/raise a domain error for invalid state (already started/ended), insufficient players, or unauthorized caller.
  - Do not mutate game state on failure.
### Method: new_turn()
- **What:**
  - Method to initiate and for current player to take their turn.
- **Why:**
  - Encapsulates all logic of a player's turn.
- **How:**
  - Accepts index_of_current_player as parameter to identify current player and place in turn order. 
  - Call handle_action('MOVE) to roll dice, move piece
  - resolve_space.py to identify space landed on
  - show space card center screen? Below image of card are two buttons (if applicable): Buy and Auction. HOW TO HANDLE INPUT?
  - run space.py's related procedure - to represent flow of actions offered to player through prompts: Do you want to buy this property? This property will go to auction. Do you want to bid? Input bid. Do you want to sell? 
  - Call new turn

trading - make offer button. other player can accept
